...
import DiscountTableFiltersResult from '../discount-table-filters-result';
// api
import { useFetchDiscount, useMutationDelete } from 'src/utils/discount';
...
export default function DiscountView() {
  ...
  const { data, productsLoading, productsEmpty } = useFetchDiscount();
  ...
  const handleDeleteRow = useCallback(
    ...
    [DeleteDiscount, dataInPage.length, table, tableData]
  );

  const handleDeleteRows = useCallback(() => {
    const selectedIds = table.selected;

    Promise.all(selectedIds.map((id) => DeleteDiscount(id))).then(() => {
  ...
  }, [DeleteDiscount, dataFiltered.length, dataInPage.length, table, tableData]);

  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.discount.edit(id));
    },
    [router]
  );
        ...
        <CustomBreadcrumbs
          ...
          action={
            <Button
              ...
              href={paths.dashboard.discount.new}
              variant="contained"
